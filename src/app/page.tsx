...
const STEPS = [
  { icon: Send, text: "Ask: “open saucedemo.com and create a test plan”" },
  { icon: Zap, text: "Pantau progress: Planning → Browsing → Testing → Reporting" },
    ...
    <div className="mesh-bg min-h-screen text-white">
      ...
      >
        ...
        <div className="ml-auto flex items-center gap-2 text-sm">
          <motion.div whileHover={reducedMotion ? undefined : { y: -2 }} whileTap={{ scale: 0.96 }}>
            <Link
            href="/chat"
      ...
      <motion.header
        initial="hidden"
        animate="show"
        variants={{ show: { transition: { staggerChildren: reducedMotion ? 0 : 0.1 } }, hidden: {} }}
      ...
      >
        ...
        <motion.div variants={reveal} className="mt-8 flex flex-wrap items-center justify-center gap-3">
          <motion.div whileHover={reducedMotion ? undefined : { y: -3, scale: 1.02 }} whileTap={{ scale: 0.97 }}>
            <Link
            href="/chat"
          ...
          <motion.div whileHover={reducedMotion ? undefined : { y: -3 }} whileTap={{ scale: 0.97 }}>
            <Link
            href="#fitur"
        ...
        <motion.p variants={reveal} className="mt-4 font-mono text-[11px] text-zinc-600">
          “open saucedemo.com and create a test plan” → document + test cases
        </motion.p>
      </motion.header>
      ...
      <section className="relative z-10 mx-auto w-full max-w-5xl px-4 py-8 sm:px-6">
        <motion.div
          ...
          variants={{ show: { transition: { staggerChildren: reducedMotion ? 0 : 0.08 } }, hidden: {} }}
          className="glass grid gap-2 rounded-3xl p-4 sm:grid-cols-4 sm:p-6"
        >
          {STEPS.map((s, i) => (
            <motion.div key={i} variants={reveal} whileHover={reducedMotion ? undefined : { y: -3 }} className="flex items-start gap-3 rounded-2xl p-3">
              <motion.span whileHover={reducedMotion ? undefined : { rotate: -8 }} className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-white text-black">
                <s.icon size={15} />
              </motion.span>
        ...
        >
          {FEATURES.map((f) => (
            <motion.div
              key={f.title}
              variants={reveal}
              whileHover={reducedMotion ? undefined : { y: -6, transition: { duration: 0.2 } }}
            ...
            >
              <motion.span whileHover={reducedMotion ? undefined : { scale: 1.08, rotate: 5 }} className="inline-grid h-10 w-10 place-items-center rounded-xl bg-white text-black">
                <f.icon size={18} />
              </motion.span>
